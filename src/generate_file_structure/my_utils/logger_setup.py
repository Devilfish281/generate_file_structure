...
# Change the console code page to UTF-8 on Windows to support Unicode characters.
if os.name == "nt":
...
def get_default_config():
    ...
    **Default Configuration Keys:**
    ...
    config = {
        ...
        "log_dir_pwd": os.getcwd(),  # Default to current directory if not provided
        "log_dir_extened": "",  # Default to empty string, can be set via LOG_DIR environment variable
        "log_dir_path": os.getcwd(),  # Default log directory is empty, will be set to current directory if LOG_DIR is not provided.
        "log_max_bytes": 5
        * 1024
...
def load_config(config_file=None):
    ...
    **Overrides:**
    ...
    config = get_default_config()  # Initialize with default values
    ############################
...
def setup_logger(logger_name=None, config_file=None, debug_messages=False):
    ...
    _debug(f"Log directory: {log_dir_path}")  # Debug statement
    _debug(f"Log file absolute path: {log_file}")  # Debug statement
